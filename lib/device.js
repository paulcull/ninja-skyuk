...
function Device(opts, app, skyBox, emitter) {
  ...
  .split(',').forEach(  function listenToNotification(eventName) {
    player.on(eventName, function(e) {
      ...
      if (!_on) {
        ...
        self.devices.mediaObject._data.state.isRecording = 'N';
      ...
      self.devices.skyHDState.emit('data',self.devices.mediaObject._data.state.state+'-'+self.devices.mediaObject._data.state.mode);
  ...
  .split(',').forEach(  function listenToNotification(eventName) {
    player.on(eventName, function(e) {
      ...
      if (self.devices.mediaObject._data.state.state ==='on') {
        ...
        //console.log(JSON.stringify(self.devices.mediaObject._data));
        self.devices.mediaObject.emit('data',self.devices.mediaObject._data);
        self.devices.nowPlaying.emit('data',self.devices.mediaObject._data.track.desc+'-'+self.devices.mediaObject._data.track.album);
  ...
  .split(',').forEach(  function listenToNotification(eventName) {
    player.on(eventName, function(e) {
      ...
      self.app.log.debug('(Sky Plus HD UK) : Got Event %s: %s',eventName, JSON.stringify(e));
      // self.devices.mediaObject._data.state.track_id = e.id;
      self.devices.mediaObject._data.state.isRecording = 'Y';      
      ...
      // console.log(JSON.stringify(self.devices.mediaObject._data));
      self.devices.mediaObject.emit('data',self.devices.mediaObject._data);
      self.devices.nowPlaying.emit('data','Recording - '+self.devices.mediaObject._data.track.desc+'-'+self.devices.mediaObject._data.track.album);
  ...
  function displayText(_title) {
    ...
    this.D = 240;
    this.mac = self.mac;
    this.name = 'skyHD-T-'+this.textTitle.toUpperCase()+'-'+self.mac.replace(/[^a-zA-Z0-9]/g, '');
    this.G = 'skyHD'+this.textTitle.toUpperCase()+self.mac.replace(/[^a-zA-Z0-9]/g, '');
  }
  ...
  displayText.prototype.write = function(data) {
    ...
    if (this.textTitle === 'commander') {
      ...
      case 'rew':
        ...
        break;
      case 'play':
        self.app.log.debug('Sky Plus HD UK - Play');
        player.play();
        break;
      case 'pause':
        self.app.log.debug('Sky Plus HD UK - Pause');
        player.pause();    
        break;    
      case 'playPause':
  ...
  function mediaObject() {
    ...
    self.app.log.info('Sky Plus HD UK - media object [%s] being created.', this.G);    
  }
  ...
  this.devices = {
    ...
    skyHDState: new displayText('skyHDState'),
